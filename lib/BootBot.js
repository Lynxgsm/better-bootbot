...
class BootBot extends EventEmitter {
  ...
  start(port) {
    ...
    this.server = this.app.listen(this.app.get("port"), () => {
      ...
      console.log(
        `Facebook Webhook running on localhost:${portNum}${this.webhook}`
      );
    });
  }
  ...
  sendRequest(body, endpoint, method) {
    ...
    method = method || "POST";
    return fetch(
      `https://graph.facebook.com/${this.graphApiVersion}/me/${endpoint}?access_token=${this.accessToken}`,
      {
        method,
        headers: {
  ...
  conversation(recipientId, factory) {
    ...
    this._conversations.forEach((convo) => {
      if (recipientId && recipientId === convo.userId && convo.isActive()) {
        exists = true;
      }
    });

    if (exists) {
      this._conversations.forEach((c, index) => {
        if (recipientId && recipientId === c.userId && c.isActive()) {
          this._conversations[index].end();
        }
      });
    }
  ...
  _initWebhook() {
    this.app.get(this.webhook, (req, res) => {
      if (
        ...
        res.status(200).json(parseInt(req.query["hub.challenge"]));
      } else {
        console.error(
